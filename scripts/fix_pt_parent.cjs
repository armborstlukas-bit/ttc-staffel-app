...
// Find the MobileBottomNav with unreadCount={0} (unique to parent/youth view)
const marker = `        {isMobile && <MobileBottomNav view={view} navTo={navTo} userRole={userRole} canEdit={canEdit} appSettings={appSettings} unreadCount={0}/>}\n      </div>\n    );\n  }`;
              ...
              <div style={{margin:'0 16px',background:'rgba(167,139,250,0.05)',border:'1px solid rgba(167,139,250,0.15)',borderRadius:'16px',padding:'12px 14px',display:'grid',gap:'8px'}}>
                {myPTs.map(pt=>{
                  ...
                  if(pt.finalStandings){
                    ...
                    const stats4=pt.players.map((_,i)=>({idx:i,wins:0,losses:0,setsWon:0,setsLost:0}));
                    pt.matches.forEach(m=>{if(!m.result)return;const{sets1,sets2}=m.result;stats4[m.p1Idx].setsWon+=sets1;stats4[m.p1Idx].setsLost+=sets2;stats4[m.p2Idx].setsWon+=sets2;stats4[m.p2Idx].setsLost+=sets1;if(sets1>sets2)stats4[m.p1Idx].wins++;else stats4[m.p2Idx].wins++;});
                    const sorted4=[...stats4].sort((a,b)=>b.wins!==a.wins?b.wins-a.wins:(b.setsWon-b.setsLost)-(a.setsWon-a.setsLost));
                    const myIdx4=pt.players.findIndex(p=>p.childId===myChild.id);
                    const myRank4=sorted4.findIndex(s=>s.idx===myIdx4);
                    const myStat4=stats4[myIdx4]||{wins:0,setsWon:0,setsLost:0};
                    const myMatchCount=pt.matches.filter(m=>m.result&&(m.p1Idx===myIdx4||m.p2Idx===myIdx4)).length;
                    myEntry={place:myRank4+1,wins:myStat4.wins,losses:myMatchCount-myStat4.wins,setsWon:myStat4.setsWon,setsLost:myStat4.setsLost};
                  }
