...
export default function TrainingsApp() {
  ...
  // Datenstruktur:
  // subgroups: { [subgroupId]: { id, name, groupId, trainingDates: [date,...] } }
  // children:  { [childId]: { id, name, subgroupId, attendance: { [date]: status } } }
  ...
  const saveSubgroups = (updated) => {
    setSubgroups(updated);
    setDoc(doc(db, 'ttc', 'subgroups'), updated);
  };
  const saveChildren = (updated) => {
    setChildren(updated);
  ...
  const getSubgroupsForGroup = (groupId) =>
    Object.values(subgroups).filter(s => s.groupId === groupId);

  const getChildrenForSubgroup = (subgroupId) =>
    Object.values(children).filter(c => c.subgroupId === subgroupId);
  ...
  const getAttendanceStats = (childId, subgroupId) => {
    ...
    return { present, unexcused, excused, total, percent: total > 0 ? Math.round((present / total) * 100) : 0 };
  };
  ...
  const deleteSubgroup = (subId) => {
    if (!window.confirm('Untergruppe löschen? Kinder bleiben erhalten, haben aber keine Gruppe mehr.')) return;
  ...
  const deleteChild = (childId) => {
    if (!window.confirm('Kind wirklich löschen? Alle Anwesenheitsdaten gehen verloren!')) return;
  ...
  // Training-Tag hinzufügen (wenn noch nicht vorhanden)
  ...
  const exportToCSV = () => {
    let csv = `TTC Grün-Weiß Staffel - Anwesenheitsauswertung\nErstellt am,${new Date().toLocaleDateString('de-DE')}\n\n`;
    FIXED_GROUPS.forEach(group => {
      ...
      subs.forEach(sub => {
        ...
        csv += `Name,${dates.join(',')},Anwesend,Fehlzeiten,Gesamt,Quote\n`;
        kids.forEach(child => {
          const stats = getAttendanceStats(child.id, sub.id);
          const statusRow = dates.map(d => {
            const s = (child.attendance || {})[d];
            return s === 'present' ? 'A' : s === 'absent_unexcused' ? 'U' : s === 'absent_excused' ? 'E' : '-';
          }).join(',');
          csv += `"${child.name}",${statusRow},${stats.present},${stats.unexcused + stats.excused},${stats.total},${stats.percent}%\n`;
        });
        csv += '\n';
      });
    });
    ...
    link.download = `TTC_Anwesenheit_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  if (!user) return (
    <div style={{ ...s.page, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
      <div style={{ background:'white', borderRadius:'16px', padding:'40px', maxWidth:'400px', width:'100%', boxShadow:'0 10px 40px rgba(0,0,0,0.2)' }}>
        ...
        <form onSubmit={authMode==='login' ? async(e)=>{ e.preventDefault(); setError(''); try{ await signInWithEmailAndPassword(auth,loginEmail,loginPassword); }catch{ setError('Login fehlgeschlagen!'); }} : async(e)=>{ e.preventDefault(); setError(''); try{ await createUserWithEmailAndPassword(auth,loginEmail,loginPassword); }catch{ setError('Registrierung fehlgeschlagen. Min. 6 Zeichen!'); }}} style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
          ...
          <button type="submit" style={{ padding:'12px', background:'#358941', color:'white', border:'none', borderRadius:'8px', fontSize:'16px', fontWeight:'600', cursor:'pointer' }}>{authMode==='login'?'Anmelden':'Registrieren'}</button>
        </form>
        <div style={{ marginTop:'20px', textAlign:'center' }}>
          <button onClick={()=>{ setAuthMode(authMode==='login'?'register':'login'); setError(''); }} style={{ background:'none', border:'none', color:'#358941', cursor:'pointer', fontSize:'14px', textDecoration:'underline' }}>{authMode==='login'?'Neu registrieren':'Zum Login'}</button>
        </div>
      </div>
    </div>
  );
  ...
  const Header = ({ back, backLabel, backAction }) => (
    <div style={{ ...s.card, display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'12px' }}>
      ...
      <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
        <input type="date" value={trainingDate} onChange={e=>setTrainingDate(e.target.value)} style={{ padding:'8px 12px', border:'1px solid #ddd', borderRadius:'6px', fontSize:'14px' }} />
        <button onClick={exportToCSV} style={s.btn('#358941')}><Download size={16}/> CSV</button>
        ...
        {FIXED_GROUPS.map(group => {
          ...
          return (
            <div key={group.id} onClick={()=>{ setActiveGroup(group); setView('group'); }}
              style={{ ...s.card, cursor:'pointer', borderLeft:`6px solid ${group.color}`, display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:0, transition:'transform 0.1s' }}
      ...
      <div style={s.page}><div style={s.wrap}>
        ...
        <div style={s.card}>
          ...
          <div style={{ display:'flex', gap:'8px', marginBottom:'20px' }}>
            <input style={s.input} placeholder="Neue Trainingsgruppe (z.B. U11, Montag-Gruppe...)" value={newSubgroupName} onChange={e=>setNewSubgroupName(e.target.value)} onKeyPress={e=>e.key==='Enter'&&addSubgroup()} />
          ...
          <div style={{ display:'grid', gap:'12px' }}>
            {subs.length === 0
              ...
              : subs.map(sub => {
                ...
                const trainings = (sub.trainingDates||[]).length;
                return (
                  <div key={sub.id} style={{ border:'1px solid #ddd', borderRadius:'10px', overflow:'hidden' }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px', background:'#f8f9fa', cursor:'pointer' }}
                      ...
                      <div>
                        ...
                        <p style={{ margin:0, color:'#666', fontSize:'12px' }}>{kids.length} Kinder · {trainings} Trainings · heute {presentToday} anwesend</p>
                      </div>
                      <div style={{ display:'flex', gap:'8px' }}>
                        <button onClick={e=>{ e.stopPropagation(); deleteSubgroup(sub.id); }} style={{ padding:'6px', background:'#fee2e2', border:'none', borderRadius:'6px', cursor:'pointer', color:'#dc2626' }}><Trash2 size={16}/></button>
        ...
        <div style={s.card}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px', flexWrap:'wrap', gap:'8px' }}>
            ...
            <span style={{ fontSize:'13px', color:'#666', background:'#f3f4f6', padding:'4px 10px', borderRadius:'20px' }}>
              Training: {new Date(trainingDate+'T12:00:00').toLocaleDateString('de-DE', { weekday:'short', day:'2-digit', month:'2-digit', year:'numeric' })}
            </span>
          </div>
          ...
          <div style={{ display:'grid', gap:'10px' }}>
            {kids.length === 0
              ...
              : kids.map(child => {
                ...
                const stats = getAttendanceStats(child.id, sub.id);
                return (
                  <div key={child.id} style={{ padding:'14px', borderRadius:'10px', border:'1px solid #ddd', background: todayStatus==='present'?'#f0fdf4':todayStatus==='absent_unexcused'?'#f9fafb':todayStatus==='absent_excused'?'#fffbeb':'white' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
                      <div style={{ flex:1, minWidth:'120px' }}>
                        <p style={{ margin:'0 0 2px', fontWeight:'600', color:'#333', fontSize:'16px' }}>{child.name}</p>
                        <p style={{ margin:0, fontSize:'12px', color:'#999' }}>{stats.total} Trainings · {stats.present}x da · {stats.percent}% Anwesenheit</p>
                      </div>

                      {/* 3 Status-Buttons */}
                      <div style={{ display:'flex', gap:'6px' }}>
                        <button onClick={()=>setStatus(child.id,'present')} title="Anwesend"
                          style={{ width:'44px', height:'44px', border:`2px solid #16a34a`, background:todayStatus==='present'?'#16a34a':'white', color:todayStatus==='present'?'white':'#16a34a', borderRadius:'8px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', fontWeight:'700' }}>
                          <Check size={22}/>
                        </button>
                      ...
                      <div style={{ marginTop:'12px', padding:'12px', background:'#f0f9ff', borderRadius:'8px', border:'1px solid #bae6fd' }}>
                        ...
                        <div style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
                          {allSubgroups.filter(sg => sg.id !== sub.id).map(sg => {
                            ...
                            return (
                              <button key={sg.id} onClick={()=>moveChild(child.id, sg.id)}
                                style={s.btn(grp?.color||'#358941',undefined,true)}>
                                {grp?.emoji} {sg.name}
                              </button>
                            );
                          })}
        ...
        <div style={s.card}>
          ...
          <div style={{ marginBottom:'24px' }}>
            ...
            <div style={{ background:'#f3f4f6', borderRadius:'99px', height:'12px', overflow:'hidden' }}>
              <div style={{ width:`${stats.percent}%`, height:'100%', background: stats.percent >= 80 ? '#16a34a' : stats.percent >= 60 ? '#d97706' : '#dc2626', borderRadius:'99px', transition:'width 0.5s' }} />
            </div>
          </div>
              ...
              {dates.map(date => {
                ...
                return (
                  <div key={date} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', background: cfg?.bg||'#f9fafb', borderRadius:'8px', border:'1px solid #eee' }}>
                    <span style={{ fontSize:'14px', color:'#333' }}>{new Date(date+'T12:00:00').toLocaleDateString('de-DE', { weekday:'long', day:'2-digit', month:'2-digit', year:'numeric' })}</span>
